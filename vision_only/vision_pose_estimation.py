...
def get_cap():
    ...
    # cap.set(cv2.CAP_PROP_BRIGHTNESS, 127)
...
def load_marker_positions():
    try:
        with open("../markers_calibration/params/calibrated_marker_positions.json", "r") as f:
...
def run_tracker():
    ...
    cv2.resizeWindow("Tracker", 1920, int(1920 * 1080 / 1920))
    camera_matrix, dist_coeffs = read_camera_parameters(
        "../camera_calibration/params/camera_params.yml"
    )
    ...
    while True:
        ...
        if keypress == ord("q"):
            ...
            with open("./data/tip_pose.pkl", "wb") as f:
                pickle.dump(tip_poses, f)
            with open("./data/imu_pose.pkl", "wb") as f:
                pickle.dump(imu_poses, f)
            tip_poses = []
            imu_poses = []
        ...
        if result is not None:
            ...
            tip_pose = {
                "timestamp": time.time(),
                "position": {
                    "x" : tvec_tip_relative[0][0],
                    "y" : tvec_tip_relative[1][0],
                    "z" : tvec_tip_relative[2][0]
                },
                "orientation": {
                    "x" : R_tip_relative[0][0],
                    "y" : R_tip_relative[1][0],
                    "z" : R_tip_relative[2][0],
                    "w" : R_tip_relative[0][1]
                }
            }
            
            tip_poses.append(tip_pose)
            imu_poses.append(CameraReading(tvec_relative, R_relative).to_json())
